...
class TestJustwatchDirect:

    def test_should_direct_load_justwatch(self):
        setup = _justwatch_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-justwatch", "live" if setup["live"] else "unit")
        if _skip:
...
def _justwatch_direct_setup(mockres):
