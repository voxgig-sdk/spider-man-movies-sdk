...
class TestMediaDirect:

    def test_should_direct_load_media(self):
        setup = _media_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-media", "live" if setup["live"] else "unit")
        if _skip:
...
def _media_direct_setup(mockres):
