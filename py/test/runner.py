# SpiderManMovies SDK test runner
...
class SpiderManMoviesTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            SpiderManMoviesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = SpiderManMoviesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = SpiderManMoviesTestRunner.getenv("SPIDER_MAN_MOVIES_TEST_LIVE")
        override = SpiderManMoviesTestRunner.getenv("SPIDER_MAN_MOVIES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = SpiderManMoviesTestRunner.getenv(key)
        ...
        explain = SpiderManMoviesTestRunner.getenv("SPIDER_MAN_MOVIES_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["SPIDER_MAN_MOVIES_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if SpiderManMoviesTestRunner._test_control is not None:
            return SpiderManMoviesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                SpiderManMoviesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            SpiderManMoviesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = SpiderManMoviesTestRunner.load_test_control()
...
def load_env_local():
    SpiderManMoviesTestRunner.load_env_local()


def env_override(m):
    return SpiderManMoviesTestRunner.env_override(m)


def entity_data(v):
    return SpiderManMoviesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return SpiderManMoviesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return SpiderManMoviesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return SpiderManMoviesTestRunner.load_test_control()


def live_client_options():
    return SpiderManMoviesTestRunner.live_client_options()


def live_delay_ms():
    return SpiderManMoviesTestRunner.live_delay_ms()
