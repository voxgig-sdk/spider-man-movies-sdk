# SpiderManMovies Python SDK Reference

Complete API reference for the SpiderManMovies Python SDK.
...
#### `SpiderManMoviesSDK.test(testopts=None, sdkopts=None)`
...
Create a new `JustwatchEntity` instance. Pass `None` for no initial data.
...
```python
result = client.Justwatch().load({"q": "q"})
```
...
Create a new `JustwatchEntity` instance with the same options.
