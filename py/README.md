...
The Python SDK for the SpiderManMovies API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Justwatch(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.SpiderManMovies(None).load(
    {"id": "test01"}, None
)
