...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = SpiderManMoviesOmni.make_runner(TEST_JSON_FILE, SpiderManMoviesSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    SpiderManMoviesError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : SpiderManMoviesOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = SpiderManMoviesOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
