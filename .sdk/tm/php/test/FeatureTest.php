...
// SpiderManMovies SDK feature test
//
...
class FtCtrl extends SpiderManMoviesControl
{
...
class FtClient
{
    ...
    private SpiderManMoviesUtility $utility;

    public function __construct(SpiderManMoviesUtility $utility, array $options)
    {
    ...
    public function get_utility(): SpiderManMoviesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (SpiderManMoviesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public SpiderManMoviesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new SpiderManMoviesUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (SpiderManMoviesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = SpiderManMoviesConfig::make_feature($name);
    ...
    private static function buildUrl(SpiderManMoviesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof SpiderManMoviesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new SpiderManMoviesResponse($fetched);
        }
    ...
    private function fail(SpiderManMoviesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (SpiderManMoviesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
