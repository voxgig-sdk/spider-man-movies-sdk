...
// SpiderManMovies SDK retry feature
...
class SpiderManMoviesRetryFeature extends SpiderManMoviesBaseFeature
{
    ...
    public function init(SpiderManMoviesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (SpiderManMoviesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(SpiderManMoviesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(SpiderManMoviesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
