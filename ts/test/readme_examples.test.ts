...
//     (\`new SpiderManMoviesSDK(...)\` / \`SpiderManMoviesSDK.test(...)\` become
//     \`SpiderManMoviesSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'SpiderManMoviesSDK'
...
const TEST_SEED = {"entity":{"justwatch":{"test01":{"id":"test01"}},"media":{"test01":{"id":"test01"}},"photo":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new SpiderManMoviesSDK(...)\` and any
// \`SpiderManMoviesSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+SpiderManMoviesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/SpiderManMoviesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
