...
describe('JustwatchDirect', async () => {
  ...
  test('direct-load-justwatch', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-justwatch', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SPIDER_MAN_MOVIES_TEST_JUSTWATCH_ENTID']
