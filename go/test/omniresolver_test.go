...
import (
	...
	omni "github.com/voxgig-sdk/spider-man-movies-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.SpiderManMoviesError); is {
				out := map[string]any{
					"name":    "SpiderManMoviesError",
...
func omniCtx(arg any, client *sdk.SpiderManMoviesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.SpiderManMoviesSDK); is {
			client = live
