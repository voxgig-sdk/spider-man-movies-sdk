...
func TestMediaEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set SPIDER_MAN_MOVIES_TEST_MEDIA_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		mediaRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.media")))
...
func mediaBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("SPIDER_MAN_MOVIES_TEST_MEDIA_ENTID")
	...
	idmapResolved := core.ToMapAny(env["SPIDER_MAN_MOVIES_TEST_MEDIA_ENTID"])
