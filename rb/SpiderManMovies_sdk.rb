...
class SpiderManMoviesSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = SpiderManMoviesHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = SpiderManMoviesHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, SpiderManMoviesFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    SpiderManMoviesUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    params = SpiderManMoviesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = SpiderManMoviesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = SpiderManMoviesHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = SpiderManMoviesError.new(
        "graphql_error", "SpiderManMoviesSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = SpiderManMoviesSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
