...
module SpiderManMoviesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = SpiderManMoviesResponse.new({})
    ...
    if fetch_err
      ...
      response = SpiderManMoviesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = SpiderManMoviesResponse.new(fetched)
    else
