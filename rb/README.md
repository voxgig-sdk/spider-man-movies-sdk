# SpiderManMovies Ruby SDK



The Ruby SDK for the SpiderManMovies API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Justwatch` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
- Releases: [https://github.com/voxgig-sdk/spider-man-movies-sdk/releases](https://github.com/voxgig-sdk/spider-man-movies-sdk/releases)
...
```ruby
client = SpiderManMoviesSDK.test(testopts, sdkopts)
```
...
raise a `SpiderManMoviesError` (a `StandardError` subclass), so wrap
...
```
rb/
├── SpiderManMovies_sdk.rb       -- Main SDK module
...
The main module (`SpiderManMovies_sdk`) exports the SDK class
