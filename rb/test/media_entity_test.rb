...
class MediaEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SPIDER_MAN_MOVIES_TEST_MEDIA_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    media_ref01_data_dt0_load_result = Helpers.to_map(media_ref01_data_dt0_loaded.respond_to?(:data_get) ? media_ref01_data_dt0_loaded.data_get : media_ref01_data_dt0_loaded)
    assert !media_ref01_data_dt0_load_result.nil?
    assert_equal media_ref01_data_dt0_load_result["id"], media_ref01_data["id"]
...
def media_basic_setup(extra)
  ...
  entid_env_raw = ENV["SPIDER_MAN_MOVIES_TEST_MEDIA_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["SPIDER_MAN_MOVIES_TEST_MEDIA_ENTID"])
