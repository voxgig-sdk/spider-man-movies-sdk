...
class MediaDirectTest < Minitest::Test
  def test_direct_load_media
    setup = media_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-media", setup[:live] ? "live" : "unit")
    if _should_skip
...
def media_direct_setup(mockres)
