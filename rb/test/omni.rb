...
module SpiderManMoviesOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if SpiderManMoviesOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = SpiderManMoviesOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| SpiderManMoviesOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || SpiderManMoviesOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = SpiderManMoviesSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = SpiderManMoviesError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { SpiderManMoviesOmni.errify(err) }
