...
require("spider-man-movies_types")
...
local SpiderManMoviesSDK = {}
SpiderManMoviesSDK.__index = SpiderManMoviesSDK
...
SpiderManMoviesSDK._make_feature = _make_feature


function SpiderManMoviesSDK.new(options)
  local self = setmetatable({}, SpiderManMoviesSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function SpiderManMoviesSDK:options_map()
...
function SpiderManMoviesSDK:get_utility()
  return Utility.copy(self._utility)
end


function SpiderManMoviesSDK:get_root_ctx()
  return self._rootctx
end


function SpiderManMoviesSDK:prepare(fetchargs)
...
function SpiderManMoviesSDK:direct(fetchargs)
...
function SpiderManMoviesSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function SpiderManMoviesSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "SpiderManMoviesSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function SpiderManMoviesSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function SpiderManMoviesSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "SpiderManMoviesSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function SpiderManMoviesSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = SpiderManMoviesSDK.new(sdkopts)
  sdk.mode = "test"
