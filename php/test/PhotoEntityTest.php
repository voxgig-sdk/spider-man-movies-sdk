...
class PhotoEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SPIDER_MAN_MOVIES_TEST_PHOTO_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function photo_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["photo01", "photo02", "photo03"] as $k) {
    ...
    $entid_env_raw = getenv("SPIDER_MAN_MOVIES_TEST_PHOTO_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["SPIDER_MAN_MOVIES_TEST_PHOTO_ENTID"]);
