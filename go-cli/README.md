...
boru-driven command-line client **and** interactive REPL for the SpiderManMovies
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/spider-man-movies-cli)
make build

# 2. See usage (words, entities, env vars)
./spider-man-movies-cli --help

# 3. Provide credentials once, via the environment
export SPIDER_MAN_MOVIES_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./spider-man-movies-cli load 1 justwatch            # {id:1} shorthand
./spider-man-movies-cli load '{id:1}' justwatch       # explicit match map

# 5. Override the API base URL for a single call
SPIDER_MAN_MOVIES_BASE=https://api.example.com ./spider-man-movies-cli load 1 justwatch

# 6. No arguments -> interactive REPL
./spider-man-movies-cli
spider-man-movies> load 1 justwatch
spider-man-movies> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/spider-man-movies-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/spider-man-movies-cli load 1 justwatch
   ```
...
```sh
./spider-man-movies-cli load 1 justwatch          # scalar shorthand for {id:1}
./spider-man-movies-cli load '{id:1}' justwatch     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `spider-man-movies>`). Each line is
evaluated as its own boru expression:

```text
$ ./spider-man-movies-cli
spider-man-movies> load 1 justwatch
spider-man-movies> /help
spider-man-movies> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `justwatch`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/spider-man-movies-cli`. |
...
(`./spider-man-movies-cli <expr>`) and interactively (the REPL), and expressions compose the
