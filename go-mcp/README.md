# spider-man-movies-mcp

[MCP](https://modelcontextprotocol.io) server exposing the SpiderManMovies SDK as
two agent tools — `spider-man-movies_list` and `spider-man-movies_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/spider-man-movies-mcp)
make build

# 2. Provide credentials via the environment
export SPIDER_MAN_MOVIES_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user spider-man-movies \
  -- /absolute/path/to/spider-man-movies-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./spider-man-movies-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// spider-man-movies_list: first page of records
{ "entity": "justwatch" }
{ "entity": "justwatch", "query": { } }

// spider-man-movies_load: one record by id
{ "entity": "justwatch", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/spider-man-movies-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user spider-man-movies \
     -- "$PWD"/dist/*/spider-man-movies-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `spider-man-movies_list` and `spider-man-movies_load` tools now appear
   in new sessions. Ask the agent to *"list justwatch using spider-man-movies"*
   and it calls `spider-man-movies_list` with `{"entity":"justwatch"}`.
...
```sh
export SPIDER_MAN_MOVIES_APIKEY=sk_live_xxx            # API key
export SPIDER_MAN_MOVIES_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `spider-man-movies_list` tool
...
### Call the `spider-man-movies_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `spider-man-movies_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `spider-man-movies_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SPIDER_MAN_MOVIES_APIKEY` | API key sent with every request. |
| `SPIDER_MAN_MOVIES_BASE` | Optional override of the API base URL. |
...
```sh
./spider-man-movies-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"spider-man-movies_load","arguments":{"entity":"justwatch","query":{"id":1}}}}'
```
